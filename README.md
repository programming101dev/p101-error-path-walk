...
5. feed each emitted resource log to `resource-tracker`;
...
- `-r <resource-tracker>` chooses the analyzer executable. The default is
  `resource-tracker` through `PATH`.
...
`resource-tracker` found leaks or bad releases, and `2` when the walker,
